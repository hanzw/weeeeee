...
public class TSEvaluation {
   ...
   */
  private List<ErrorModule> m_predictionsForTestData;
  ...
  public ErrorModule getPredictionsForTestData(int stepNumber) throws Exception {
    if (getM_predictionsForTestData() == null) {
      throw new Exception("No predictions for test data available!");
    }

    int numSteps = getM_predictionsForTestData().size();
    if (stepNumber > getM_predictionsForTestData().size()) {
      throw new Exception("Only predictions up to " + numSteps
          + (numSteps > 1 ? "steps" : "step") + "-ahead are available");
    }

    ErrorModule m = getM_predictionsForTestData().get(stepNumber - 1);
  ...
  public void evaluateForecaster(TSForecaster forecaster, boolean buildModel,
    ...
    setM_predictionsForTestData(null);
    ...
    if (m_evaluateTestData) {
      ...
      // set up training set prediction and eval modules
      setM_predictionsForTestData(new ArrayList<ErrorModule>());
      m_metricsForTestData = new HashMap<String, List<TSEvalModule>>();
      setupEvalModules(getM_predictionsForTestData(), m_metricsForTestData,
          AbstractForecaster.stringToList(forecaster.getFieldsToForecast()));
      ...
      for (int i = predictionOffsetForTestData; i < m_testData.numInstances(); i++) {
        ...
        updateEvalModules(getM_predictionsForTestData(), m_metricsForTestData,
  ...
  public JPanel graphPredictionsForStepsOnTesting(GraphDriver driver,
    ...
    JPanel result = driver.getGraphPanelSteps(forecaster,
        getM_predictionsForTestData(), targetName, stepsToPlot,
  ...
  public JPanel graphPredictionsForTargetsOnTesting(GraphDriver driver,
    ...
    ErrorModule preds = getM_predictionsForTestData().get(graphStepNum - 1);
...
public List<ErrorModule> getM_predictionsForTestData() {
	return m_predictionsForTestData;
}

public void setM_predictionsForTestData(List<ErrorModule> m_predictionsForTestData) {
	this.m_predictionsForTestData = m_predictionsForTestData;
}
}
