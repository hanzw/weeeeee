...
public class ErrorModule extends TSEvalModule {
  ...
  private List<List<NumericPrediction>> m_predictions;
  ...
  public void reset() {
    if (m_targetFieldNames != null) {
      setM_predictions(new ArrayList<List<NumericPrediction>>());
      ...
      for (int i = 0; i < m_targetFieldNames.size(); i++) {
        ...
        getM_predictions().add(predsForTarget);
      }
    }
  }
  ...
  public void evaluateForInstance(List<NumericPrediction> forecasts, Instance inst) 
    ...
    for (int i = 0; i < m_targetFieldNames.size(); i++) {
      ...
      getM_predictions().get(i).add(pred);
  ...
  public double[] calculateMeasure() throws Exception {
    if (getM_predictions() == null || getM_predictions().get(0).size() == 0) {
      throw new Exception("No predictions have been seen yet!");
    }
    
    double[] result = new double[m_targetFieldNames.size()];
    for (int i = 0; i < m_targetFieldNames.size(); i++) {
      List<NumericPrediction> preds = getM_predictions().get(i);
...
public List<List<NumericPrediction>> getM_predictions() {
	return m_predictions;
}

public void setM_predictions(List<List<NumericPrediction>> m_predictions) {
